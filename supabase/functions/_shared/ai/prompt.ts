...
export function buildSystemPrompt(ctx: AgentContext): string {
  const firstContact = ctx.isFirstContact
    ? `# Primeiro contato
Esta é a PRIMEIRA mensagem do cliente. ${ctx.agent.greeting_enabled
      ? `Recepcione calorosamente com o período do dia ("${ctx.period === "manhã" ? "Bom dia" : ctx.period === "tarde" ? "Boa tarde" : "Boa noite"}!") e se apresente: você é ${ctx.agent.agent_name}, assistente de suporte da ${ctx.companyName}. Se o cliente já disser o problema na primeira mensagem, NÃO pule a recepção: saúde e na sequência já aborde o problema com empatia. Se for genérica, se coloque à disposição.`
