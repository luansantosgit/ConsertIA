...
export const toolDefinitions = [
  {
    ...
    function: {
      name: "find_part",
      description: "Busca uma pe+ºa/servi+ºo no cat+ílogo da empresa (produtos) e retorna pre+ºo e estoque reais. Use ANTES de falar qualquer valor.",
      parameters: {
        type: "object",
        properties: {
          brand: { type: "string", description: "Marca do aparelho, ex: Samsung" },
          model: { type: "string", description: "Modelo do aparelho, ex: Galaxy S23" },
          part_type: { type: "string", description: "Tipo de pe+ºa/servi+ºo, ex: tela, vidro, bateria, conector de carga" },
        },
        required: ["part_type"],
      },
    },
  },
  {
    ...
    function: {
      name: "build_quote",
      description: "Monta o or+ºamento final com o pre+ºo da pe+ºa + m+úo de obra conforme a configura+º+úo da empresa. Retorne o texto exato ao cliente.",
      parameters: {
        type: "object",
        properties: {
          part_id: { type: "string", description: "ID da pe+ºa retornada pelo find_part" },
          service_type: { type: "string", description: "Descri+º+úo do servi+ºo, ex: Troca de tela" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Envia os templates diferenciais da empresa (credibilidade) antes do or+ºamento. Chame uma vez, antes do build_quote.",
  ...
  },
  {
    ...
    function: {
      name: "get_os_status",
      description: "Retorna as ordens de servi+ºo em andamento deste cliente.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Cria uma ordem de servi+ºo (OS) para a manuten+º+úo agendada. Chame ap+¦s confirmar data com o cliente. Informe o part_id para registrar a pe+ºa e o valor de m+úo de obra na OS.",
      parameters: {
        type: "object",
        properties: {
          subject: { type: "string", description: "T+¡tulo curto, ex: Troca de tela - iPhone 11" },
          description: { type: "string", description: "Descri+º+úo com o problema relatado" },
          budget_amount: { type: "number", description: "Valor total do or+ºamento aprovado" },
          part_id: { type: "string", description: "ID da pe+ºa usada no or+ºamento (retornado pelo find_part)" },
        },
  ...
  },
  {
    ...
    function: {
      name: "schedule_event",
      description: "Agenda a manuten+º+úo no calend+írio da empresa. Chame depois de create_service_order.",
      parameters: {
        type: "object",
        properties: {
          date: { type: "string", description: "Data no formato YYYY-MM-DD, futura" },
          start_time: { type: "string", description: "Hor+írio no formato HH:MM, padr+úo 09:00" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Transfere a conversa para um atendente humano. Chame ao finalizar o atendimento ou quando n+úo puder resolver.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          name: { type: "string", description: "Nome do cliente, apenas o nome (ex: Maria, Jo+úo Silva)" },
        },
        required: ["name"],
      },
    },
...
async function findPart(ctx: AgentContext, args: any): Promise<ToolResult> {
  ...
  if (rows.length === 0) {
    return {
      ok: true,
      found: false,
      message: "Nenhuma pe+ºa encontrada no cat+ílogo. N+âO informe ao cliente que a pe+ºa n+úo existe ou est+í indispon+¡vel. Diga com naturalidade: 'Vou te passar para o nosso time t+®cnico e eles v+úo analisar de perto o caso do seu aparelho.' e chame handoff_to_human em seguida.",
    };
  }
...
async function buildQuote(ctx: AgentContext, args: any): Promise<ToolResult> {
  const { data: part, error } = await ctx.supabase
    ...
    .limit(1)
    .maybeSingle();
  if (error || !part) return { ok: false, error: "Pe+ºa n+úo encontrada" };
  ...
  const valueText = ctx.quote.labor_enabled && ctx.quote.labor_mode === "separate"
    ? `${formatMoney(partPrice)} da pe+ºa + ${formatMoney(labor)} de m+úo de obra (total ${formatMoney(total)})`
    : formatMoney(total);

  const text = (ctx.quote.quote_template || "Servi+ºo: {servico}\nValor: {valor_total}")
...
async function createServiceOrder(ctx: AgentContext, args: any): Promise<ToolResult> {
  if (!ctx.agent.auto_os_enabled) return { ok: false, error: "Cria+º+úo autom+ítica de OS desativada pelo administrador." };
  if (!ctx.customer) return { ok: false, error: "Cliente n+úo vinculado +á conversa." };
...
async function scheduleEvent(ctx: AgentContext, args: any): Promise<ToolResult> {
  if (!ctx.agent.auto_schedule_enabled) return { ok: false, error: "Agendamento autom+ítico desativado pelo administrador." };
  const date = (args.date ?? "").toString();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return { ok: false, error: "Data inv+ílida. Use YYYY-MM-DD." };
  if (date < new Date().toISOString().slice(0, 10)) return { ok: false, error: "Data no passado n+úo +® permitida." };

  const { data: event, error } = await ctx.supabase
    .from("calendar_events")
    .insert({
      tenant_id: ctx.tenantId,
      title: `Manuten+º+úo - ${ctx.contactName}`,
  ...
  ctx.appointments.push({ id: event.id, title: `Manuten+º+úo - ${ctx.contactName}`, date, start_time: (args.start_time ?? "09:00").toString().slice(0, 5), os_id: args.os_id ?? null });
...
async function updateCustomerName(ctx: AgentContext, args: any): Promise<ToolResult> {
  ...
  if (name.length < 2) return { ok: false, error: "Nome inv+ílido ou muito curto." };
...
export async function executeTool(ctx: AgentContext, name: string, args: any): Promise<ToolResult> {
  ...
  switch (name) {
    ...
    case "handoff_to_human":
      ...
      return { ok: true, message: `Handoff solicitado. Encerre o atendimento com uma mensagem simp+ítica (ex: "${ctx.agent.transfer_message}") e finalize.` };
    default:
